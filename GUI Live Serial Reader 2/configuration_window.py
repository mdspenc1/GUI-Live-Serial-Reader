...
class configurationWindow(ctk.CTk):
    def __init__(self, name):
        ...
        config_to_json(self.current_config)
        ...
        self.communication_dict = {
            ...
            'serial_values': [],
            'plot_dict': self.gui_plot_dict_json_parser(),
            'exit_gui': False,
        ...
        print("gui plot cycle has been started!")
        self.after(25, write_communication_file(self.communication_dict))
        self.after(20, self.check_communication_file())
        self.after(30, self.run_module.start_gui_plot_processes())
        # self.after(1, lambda: (self.run_module.start_gui_plot_processes(), write_communication_file(self.communication_dict), self.check_communication_file()))

    def gui_plot_dict_json_parser(self):
        json_plot_dict = {}
        if not self.plot_dict:
            json_plot_dict = dict_methods.dict_to_list(json_plot_dict)
            return json_plot_dict
        json_plot_dict = {}
        for plot_key, (_, _, ax) in self.plot_dict.items():
    ...
    def check_communication_file(self):
        ...
        if (all(key in self.communication_dict for key in self.communication_keys)
            ...
            self.running = self.communication_dict['running']
            if self.communication_dict['update_config_to_json']: # Condition to handle when the backend requests the gui to update config.json with self.current_config
                print('request successfully received from process!')
                #config_to_json(self.current_config)
                ...
                self.communication_dict['update_gui_plots'] = True
                self.communication_dict['backend'] = 'requested'
                self.communication_dict['designated_author'] = 'backend'
                #write_communication_file(self.communication_dict)
                self.after(250, self.check_again())
                return
            elif self.communication_dict['generate_gui_plots']:
                print('request successfully received from process!')
                #self.generate_gui_plots()
                self.communication_dict['running'] = self.running
                if not self.running:
                    self.communication_dict['skip_to_beginning'] = True
                #self.communication_dict['plot_dict'] = self.gui_plot_dict_json_parser()
                print(f"error is after json_plot_dict_parser finishes")
    ...
    def check_again(self):
        self.after_idle(self.check_communication_file)
        return

    def generate_gui_plots(self):
        print('gui has begun to generate plots!')
        plots = self.current_config.plots
        if self.plot_dict:
            for (canvas, fig, ax) in self.plot_dict.values():
                plt.close(fig)
                ax.get_lines().clear()
            self.plot_dict = {}
        for plot_obj in plots:
            ...
            self.plot_dict[plot_obj] = (canvas, fig, ax)
        print('gui has finished generating gui plots!')
        return

    def plot_serial_data(self):
        serial_values = self.communication_dict['serial_values']
        try:
            print('gui has begun to plot serial values!')
            value_index = 0
            ...
            for plot_key, (canvas, fig, ax) in self.plot_dict.items():
            ...
            print('gui has successfully plotted serial values!')
            return
        except:
            print('gui failed to plot serial values!')
            return
...
def open_menu_window(current_win: configurationWindow, run_module: run_plots, communication_dict: dict):
    ...
    communication_dict['plot_dict'] = {}
    communication_dict['exit_gui'] = True
    ...
    communication_dict['designated_author'] = None
    write_communication_file(communication_dict)
    reset_config_json()
    if run_module.plot_cycle_termination_event.is_set():
...
def config_to_json(config: configuration):
