...
class configurationWindow(ctk.CTk):
    ...
    async def start_gui_plot_cycle(self):
        self.after(1, lambda: asyncio.create_task(self.run_module.update_gui_plots()))
        return

    async def plot_serial_data(self, values):
        try:
            value_index = 0
            ...
            asyncio.create_task(self.start_gui_plot_cycle())
            return
        except:
            return ValueError

    async def generate_gui_plots(self):
        ...
        asyncio.create_task(self.run_module.update_gui_plots())
        return
