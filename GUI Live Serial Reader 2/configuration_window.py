...
class configurationWindow(ctk.CTk):
    ...
    def restart_run_module_threads(self):
        if self.run_module.update_plot_thread.is_stopped():
            self.run_module.update_plot_thread.restart()
        if self.run_module.port_validation_thread.is_stopped():
            self.run_module.port_validation_thread.restart()
        if self.run_module.serial_data_collection_thread.is_stopped():
            self.run_module.serial_data_collection_thread.restart()
        return
    
    def stop_run_module_threads(self):
        if not self.run_module.update_plot_thread.is_stopped():
            self.run_module.update_plot_thread.stop()
        if not self.run_module.port_validation_thread.is_stopped():
            self.run_module.port_validation_thread.stop()
        if not self.run_module.serial_data_collection_thread.stop():
            self.run_module.serial_data_collection_thread.stop()
        return

    def start_gui_plot_cycle(self):
        self.restart_run_module_threads()
        self.after(1, lambda: self.run_module.update_gui_plots())
        return

    def plot_serial_data(self, values):
        self.stop_run_module_threads()
        try:
            value_index = 0
