...
mpl_run_plots = os.path.join(script_dir, 'mpl_run_plots')
os.environ['MPLCONFIGDIR'] = mpl_run_plots
import time
import serial
...
class run_plots:
    ...
    def validate_port_settings(self):
        print('Port setting validation has begun!')
        ...
        if com_port not in self.port_names:
            ...
            self.valid_port = False
            print('Port settings are invalid!')
            self.ctk_window.start_gui_plot_cycle()
            return
        ...
        if isinstance(self.serial_com, serial.Serial):                                                                   
            ...
            print('Port settings have been validated!')
    ...
    def update_gui_plots(self):
        ...
        if not plots:
            print('No plots have been added yet!')
            self.ctk_window.start_gui_plot_cycle()
            return
        if not self.plot_dict:
            print('No plot dictionary has been generated yet!')
        ...
        if not matching_plots:
            print('Mismatch detected between config plots and plot dictionary!')
            self.ctk_window.generate_gui_plots()
            return
        elif self.ctk_window.running:
            print('Data recording is running!')
