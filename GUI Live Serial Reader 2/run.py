...
class run_plots:
    ...
    def gui_plot_cycle(self, termination_event):
        ...
        # communication_dict = read_communication_file()
        # port_objs = list_ports.comports()
        # port_names = [port.device for port in port_objs]
    ...
    def update_gui_plots(self, communication_dict: dict, config: configuration):
        ...
        if not plot_dict:
            print(f'The config has no plot dict! - Running status = {communication_dict['running']}')
        ...
        if not matching_plots:
            ...
            communication_dict['backend'] = 'waiting'
            return
        elif matching_plots and communication_dict['running']:
            print(f'The plot dict and config plots match and data plotting is running! - Running status = {communication_dict['running']}')
            communication_dict['update_gui_plots'] = False
    ...
    def validate_port_settings(self, communication_dict: dict, config: configuration):
        ...
        if isinstance(self.serial_com, serial.Serial):
            if not com_port in self.port_names:
                ...
                communication_dict['validate_port_settings'] = False
                return
            print(f'serial_com baud: {self.serial_com.baudrate}, {type(self.serial_com.baudrate)}')
            print(f'config baud: {baud_rate}, {type(baud_rate)}')
            print(f'baud {self.serial_com.baudrate == baud_rate}')
