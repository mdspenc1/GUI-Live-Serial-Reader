...
class delete_serial_variables(ctk.CTkToplevel):
    ...
    def delete_serial_var(self, serial_obj):
        self.config.serial_variables.remove(serial_obj)
        if len(self.config.plots):
            for plot_obj in self.config.plots:
                for line_obj in plot_obj.lines:
                    if line_obj.x_serial == serial_obj or line_obj.y_serial == serial_obj:
                        plot_obj.lines.remove(line_obj)
                        self.config.lines.remove(line_obj)
        else:
