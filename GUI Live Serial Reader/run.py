...
class run_plots:
    def __init__(self, ctk_window):
        ...
        self.serial_reading_thread()
        self.plot_number = len(self.config.plots)

    def serial_reading_thread(self):
        if not self.ctk_window.running:
            print(f"Running value is currently {self.ctk_window.running}")
            return
        
        thread = threading.Thread(target=self.serial_reading_loop, daemon=True)
        thread.start()
