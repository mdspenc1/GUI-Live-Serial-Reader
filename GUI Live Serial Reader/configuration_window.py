...
class configurationWindow(ctk.CTk):
    ...
    def edit_csv(self, csv_name, current_config):
        ...
        if csv_path is None:
            ...
            #header = self.format_csv_headers(current_config)
            #with open(csv_name, mode="w", newline="") as file:
                #writer = csv.writer(file)
                #writer.writerows(header)

    #def format_csv_headers(self, current_config):
        #data = [[]]
        #serial_names = []
        #for serial_var in current_config.serial_variables:
            #serial_names.append(serial_var.variable_name)

        #while not set(serial_names).issubset(set(data[0])):
            #for serial_var in current_config.serial_variables:
                #if serial_var.variable_number == len(data[0]):
                    #data[0].append(serial_var.variable_name)
                #else:
                    #data[0].append("")
        #return data
