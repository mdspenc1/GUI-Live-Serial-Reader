...
class add_plots(ctk.CTkToplevel):
    def __init__(self, config):
        ...
        self.lines_frame = ctk.CTkScrollableFrame(self)
        self.lines_frame.grid(row=5, column=0)
        ...
        for line_obj in config.lines:
            ...
            checkbox = ctk.CTkCheckBox(self.lines_frame, text=f"Label: {line_obj.label} X-Units: {line_obj.x_serial.variable_units} Y-Units: {line_obj.y_serial.variable_units}", variable=var, onvalue=1, offvalue=0)
            checkbox.pack(anchor='w')
            self.line_checkboxes.append(var)
    ...
    def get_selected_lines(self, plot_obj):
        selected_lines = [i+1 for i, var in enumerate(self.chekbox_lines) if var.get()]
        valid_lines = True
        x_unit = ""
        y_unit = ""
    ...
    def calculate_valid_plot_positions(self, config):
    ...
    def add_plot_position(self, plot_obj, config, event=None):
        ...
        if plot_pos[0] == '(' and plot_pos[-1] == ')':
            plot_pos = [num.strip() for num in plot_pos[1:-2].split(",")]
        else:
            valid_pos = False
        for num in plot_pos:
        ...
        max_plot_pos = self.calculate_valid_plot_positions(config)
    ...
    def add_another_plot(self, plot_obj, config, event=None):
        self.add_plot_to_config(plot_obj, config)
...
class edit_plots_list(ctk.CTkToplevel):
    ...
    def populate_plot_list(self, config):
        ...
        for row_idx, plot_obj in enumerate(config.lines):
            parameters = [
                plot_obj.plot_title,
                plot_obj.x_label,
                plot_obj.y_label,
                plot_obj.plot_position
            ]
...
class edit_plot(ctk.CTkToplevel):
    def __init__(self, plot_obj, config):
        ...
        plot_ID = config.plots.index(plot_obj)
